...
% \begin{figure*}
% \inputtikz{EMF/EMF_tikz_de}
% \caption{Das EMF-Diagramm für die Doppler-freie Spektroskopie von Rubidium.}
% \end{figure*}
...
\begin{itemize}
	...
	\item Im Experiment werden Energieabstände zwischen Übergängen in Rubidiumatomen gemessen, die etwa 7 Größenordnungen kleiner sind als die Energie des Übergangs selbst. Dabei macht man sich den Umstand zunutze, dass man nur relative, nicht aber absolute Energien genau messen muss.
	...
	\item Das Experiment ist ein Beispiel für den iterativen Prozess der Modellierung in der Physik. Sowohl die theoretische Beschreibung des untersuchten Phänomens als auch die Messapparatur und ihre Beschreibung müssen schrittweise verfeinert werden, bis beide übereinstimmen.
...
\section{Modellierung in der Physik}


Der Kern der Physik besteht darin, Modelle zur Beschreibung von Beobachtungen zu entwickeln. Dabei kann man zwischen Modellen des Phänomens, also des physikalischen Systems, und Modellen des messenden Systems unterscheiden. Beide beruhen auf physikalischen Gesetzen und Konzepten, treffen Annahmen und vereinfachen. Zusammen beschreiben sie das, was bei der Messung passiert. Mithilfe dieser beiden Modelle können wir die Daten mit der Vorhersage vergleichen. Wenn beides übereinstimmt, sind wir fertig. Meist ist das aber nicht auf Anhieb der Fall. Dann müssen wir die Gründe für die Abweichung finden und Verbesserungen einbauen. Dies kann verschiedene Aspekte betreffen: das Messsystem, das physikalische System, oder deren Modelle. So kann das physikalische System beispielsweise verbessert werden, indem ein Pendel noch mehr als Punktmasse gestaltet wird. Alternativ kann das Modell angepasst werden, indem das Trägheitsmoment berücksichtigt wird. Auch des Messsystem kann verbessert werden, beispielsweise durch Erhöhung der Abtastrate. Ebenso kann das Modell des Messsystems angepasst werden, um Aliasing zu berücksichtigen.  Dieser iterative Prozess ist in Abbildung \ref{fig:emf_diagramm} dargestellt.


\begin{figure*}
	\inputtikz{EMF/EMF_tikz_de}
	\caption{Iterativer Prozess der Modellierung in der Physik. Übersetzt aus  \cite{Lewandowski2018}.}
	\label{fig:emf_diagramm}	
\end{figure*}

In diesem Versuch werden Sie die Hyperfeinstruktur von Rubidium untersuchen, die optischen Übergänge modellieren und die Messung mittels dopplerfreier Spektroskopie durchführen. Sie werden den Prozess so lange iterativ optimieren, bis die Abweichung zwischen Theorie und Experiment weniger als 5~MHz beträgt. Ein Beispiel ist in Abbildung~\ref{fig:demo_abweichung} dargestellt. Dazu müssen Sie die Messung sofort auswerten, um zu entscheiden, welche Veränderung Sie vornehmen wollen. Die „Fragen zur Vorbereitung” zeigen Ihnen die ersten Schritte. Erstellen Sie dabei Auswerte-Skripte in einer Programmiersprache Ihrer Wahl (z. B. Python). Bringen Sie diese lauffähig auf einem Laptop mit, um die Messung sofort auszuwerten. Da Messung und Auswertung verzahnt stattfinden, wird es wahrscheinlich so sein, dass Sie mehr als einen Tag im Labor benötigen. Der Aufbau steht Ihnen die ganze Woche zur Verfügung.
	\begin{marginfigure}[-13mm]{\Huge \textbf{!}}\end{marginfigure}
...
\begin{equation}
	\Delta E_{quadrupol} = h \,  B_{HFS}  \frac{\frac{3}{2} K (K+1)- 2 I (I+1) J (J+1)}%
...
\begin{figure}
	...
	\caption{Lage der Übergänge auf einer relativen Frequenzachse.}
...
\begin{questions}
	\myitem{Zeichnen Sie ein Diagramm, das die energetische Lage der relevanten Zustände und die erlaubten Übergänge skizziert. Geben Sie dabei sowohl die spektroskopische Bezeichnung als auch die Quantenzahlen an. Beschriften Sie die Linien in Abbildung \ref{sim_spectra_null}.}
	\myitem{Schreiben Sie ein Skript in einer Programmiersprache Ihrer Wahl, das die im Experiment erwarteten Positionen und Abstände der Linien berechnet. Zur Kontrolle: Der Mittelwert aller Linien sollte 3.229~GHz betragen, wenn die niedrigste bei Null liegt (ohne die unten eingeführten Cross-Over-Peaks). \label{frage:peaks}}
\end{questions}
...
Wir verwenden ein Fabry-Perot-Interferometer, um regelmäßige Marker auf der Frequenzachse zu erzeugen. Ein Fabry-Perot-Interferometer besteht aus zwei (teildurchlässigen) Spiegeln und entspricht damit einem Laserresonator. Es kommt zu konstruktiver Interferenz, wenn die Länge eines geschlossenen Stahlwegs im Resonators ein ganzzahliges Vielfaches der  Wellenlänge ist. Wird die Frequenz des Lasers variiert, kommt es periodisch zu konstruktiver und destruktiver Interferenz. Der spektrale Abstand wird hier freier Spektralbereich (engl. free spectral range, FSR) genannt und beträgt.
	...
	\Delta\nu_{FSR}=\frac{c_0}{L}  
	\end{equation}
mit der Lichtgeschwindigkeit $c_0$ und $L$ der Länge des Strahlwegs. Im Experiment verwenden wir einen konfokalen Resonator mit sphärischen Spiegel und gemeinsamen Brennpunkt. Dadurch ist die optische Weglänge im Resonator vier mal der Abstand der Spiegel,  so dass sich ein freier Spektralbereich von ca. 300 MHz ergibt. Im Experiment wird parallel zum Transmissionssignal durch das Rubidium-Gas die durch das Fabry-Perot-Interferometer transmittiert Leistung gemessen. Aus der periodischen Änderung dieser Leistung wird dann die relative Frequenzachse durch abschnittsweise lineare Interpolation konstruiert. Dazu ist die Python-Funktion \texttt{interp} aus \texttt{numpy} sehr hilfreich.
...
\begin{questions}
	\myitem{Plotten Sie mittels einer Programmiersprache Ihrere Wahl das erwartete Doppler-verbreiterte Absoprtionsspektrum von Rubidium in der natürlichen Isotopenmischung.
...
Die Absorption von Rubidiumgas kann durch das Lambert-Beer'sche Gesetz mit dem Absorptionskoeffizienten $\alpha$ beschrieben werden. Die Funktion $\alpha(\nu)$ nennt man Absorptionsspektrum. Die transmittierte Leistung $P$ nimmt exponentiell mit der Länge $L$ der Probe ab:
\begin{equation}
	P = P_0  \, e^{- \alpha L} \quad \text{beziehungsweise} \quad 
	\alpha = -\frac{1}{L} \ln \left( \frac{P}{P_0} \right) \quad .
	 \label{eq:lambert_beer}
\end{equation}
Im Experiment beträgt die optische Weglänge in der Gaszelle $L=75$~mm.
...
\begin{questions}
	\myitem{Plotten Sie mittels einer Programmiersprache Ihrer Wahl die Teilchenzahldichte  $\rho$  von Rubidium als Funktion der Temperatur $T$ im Bereich $10 ^\circ$C --  $70 ^\circ$C. }
	\myitem{Plotten Sie mittels einer Programmiersprache Ihrer Wahl das Diodensignal $P(\nu_0, T) / P_0$ bei konstanter Frequenz $\nu_0$ als Funktion der Temperatur $T$ im Bereich $10 ^\circ$C --  $70 ^\circ$C. $\nu_0$ sei dabei so gewählt, dass $P(\nu_0, T=293 \rm{K}) / P_0 = 0.1 $ ist. }
\end{questions}
...
\begin{equation}
	\omega_0 = \omega_L \left( 1 - \frac{v}{c_0} \right) \quad .
\end{equation}
...
\begin{questions}
	\myitem{Ergänzen Sie Ihr Skript zu den vorhergesagten Peak-Positionen aus Frage \ref{frage:peaks} um die cross-over peaks.}
\end{questions}
...
\begin{questions}
	\myitem{Die Maxima der roten Kurve in Abbildung \ref{fig:balanced}(unten)  befinden sich bei den relativen Frequenzen -103, -73, -42, -15, +17 und +80~MHz. Um welche Übergänge handelt es sich? }

	%\myitem{Die Maxima der schwarzen Kurve in Abbildung \ref{fig:balanced}  befinden sich bei den relativen Frequenzen -103, -73, -42, -15, +17 und +80~MHz.	Durch welche Werte der Dipol- und Quadrupol-Hyperfeinstrukturkonstanten  $A_{HFS}$ und $B_{HFS}$ werden diese Linien am besten beschrieben? Passen Sie dazu Ihr Skript aus Frage \ref{frage:peaks} an, um eine Veränderung der tabellierten Konstanten zu ermöglichen. Optimieren Sie diese (siehe Anhang) um die Daten optimal zu beschrieben. }
\end{questions}
	
\begin{marginfigure}
	\inputtikz{\currfiledir balanced_raw}

	\inputtikz{\currfiledir balanced}


	\caption{\textit{oben}: Intensität von Abfrage- (blau) und Referenzstrahl (rot). Der Referenzstrahl zeigt nur eine doppler-verbreiterte Linie. Im Abfrage-Strahl sind zusätzlich auch Doppler-freie Linien zu erkennen. \textit{unten}: Der balanced detector (rot) liefert durch die analoge subtraktion ein rauschärmeres Signal als die (vergrößerte) digitale Differenz der beiden obigen Kurven (blau). Grau eingezeichnet sind die erwarteten Linienpositionen. 
	\label{fig:balanced} }
\end{marginfigure}
%================================
...
\begin{questions}
	\myitem{Beschrieben Sie in Ihren Worten die Funktion der drei Strahlen Pumpstrahl, Abfragestrahl und Referenzstrahl. Gehen Sie dabei auch auf die Intensität ein.}
...
\begin{marginfigure}[10mm]{\Huge \textbf{!}}\end{marginfigure}%
Ein Experiment gelingt nur in den seltensten Fällen auf Anhieb. Oft zeigt sich erst bei der Auswertung der Daten, ob alles geklappt hat und was man besser anders gemacht hätte. Durchführung und Auswertung sind eng verzahnt. Bringen Sie deshalb ein Laptop mit Ihrer gewohnten Programmierumgebung zum Versuch mit, um die Daten direkt vor Ort auswerten zu können. Die Fragen zur Vorbereitung haben dazu schon die ersten Schritte gelegt. Bringen Sie deshalb auch Ihren Programmcode mit, der die Fragen behandelt. Wenn Sie möchten, können Sie auch den Jupyter-Hub \url{inf4all.uni-bayreuth.de} online vom Praktikumsrechner aus benutzen. Gegebenenfalls benötigen Sie auch einen USB-Stick, um die Messwerte vom Praktikumsrechner auf Ihren Laptop zu kopieren.
...
\subsection{Testmessung: Doppler-verbreitertes Spektrum}

Stellen Sie mit Hilfe der Wellenplatte die Intensität des Pumpstrahls so ein, dass er etwa 95\%  der maximalen Leistung erhalten.

Die Verstärker der Photodioden und die Analog-Digital-Wandler bewirken einen für jeden Kanal verschiedenen Spannungs-Offset. Diesen können Sie abziehen lassen, indem Sie den Knopf 'zeroing' im Justage-Tab der Software aufrufen. Dabei muss der Laser ausgeschaltet sein und danach sollte sich möglichst die Raumlicht-Situation nicht mehr ändern.
...
\begin{figure}
	\inputtikz{\currfiledir demo_abweichung}

	\caption{Bei optimierter Justage und Durchführung des Experiments gibt es nur eine geringe Abweichung zwischen gemessener und berechneter Linienposition.}
	\label{fig:demo_abweichung}
\end{figure}



\subsection{Temperaturabhängigkeit des Doppler-verbreitertes Spektrum}


Sie können die Temperatur der Rb-Gaszelle durch den Heizer variieren. Berücksichtigen Sie dabei, dass die Teilchenzahldichte exponentiell mit der Temperatur ansteigt (siehe oben bei Dampfdruckkurve). Die angezeigte Temperatur ist die des Heizelements, nicht notwendigerweise die des Gases. Auch ist Aufheizen einfacher und schneller als Abkühlen.
...
\begin{itemize} \setlength{\itemsep}{0pt}
	...
	\item Bestimmen und diskutieren Sie die Abweichung zwischen den tabellierten und gemessenen Doppler-freien Linienposition. \\[1em]
	\emph{Optimieren Sie Justage, Messung und Auswertung, bis Sie eine mittlere Abweichung von weniger als 5~MHz erreicht haben.}\\[1em]
	\begin{marginfigure}[-13mm]{\Huge \textbf{!}}\end{marginfigure}

	\item Messen und berechnen Sie das Doppler-verbreiterte Absorptionsspektrum $\alpha(\nu,T)$ bei verschiedenen Temperaturen.
	\item Bestimmen Sie aus  $\alpha(\nu_0,T) /  \alpha(\nu_0,T_0)$ über die Teilchenzahldichte die relative Änderung $T/T_0$ der Temperatur. \\[1em]
	\emph{Optimieren Sie Justage, Messung und Auswertung, um ein möglichst großes $T/T_0$ zu erreichen.}\\[1em]
	\begin{marginfigure}[-13mm]{\Huge \textbf{!}}\end{marginfigure}
	
	\item Modellieren Sie das Doppler-verbreiterte Absorptionsspektrum $\alpha(\nu,T)$ bei der größten und der kleinsten von Ihnen gemessenen Temperatur und vergleichen Sie das Ergebnis mit Ihren Erwartungen. Dazu benötigen Sie die Entartung der Ausgangs- und Ziel-Zustände, die sich aus den Orientierungsmöglichkeiten der Drehimpulse ergibt. Diese finden Sie im Anhang B von \cite{Siddons2008}.
...
Dokumentieren Sie bitte Ihr Vorgehen und die von Ihnen zur Auswertung verwendeten (Python-)Skripte. Begründen Sie Ihre Entscheidungen. Diskutieren Sie die Ergebnisse, insbesondere wenn sie von Ihren Erwartungen abweichen. Der Anhang sollte alle verwendeten Skripte umfassen. Zitieren Sie die verwendeten Quellen, auch bei Programmcode.
